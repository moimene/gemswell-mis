...
type Message = {
  ...
  /** answer generated by a fallback provider because the OpenAI primary path was unavailable */
  fallback?: boolean
}
...
function providerLabel(provider?: Message['provider']): string {
        ...
        while ((sep = buffer.indexOf('\n\n')) !== -1) {
          ...
          if (event === 'progress') {
            ...
            setMessages([...newMessages, {
              ...
              provider: payload.provider === 'openai' || payload.provider === 'anthropic' || payload.provider === 'gemini' ? payload.provider : undefined,
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] ${msg.role === 'user' ? 'order-2' : 'order-1'}`}>
              ...
              {msg.role === 'assistant' && (msg.fallback || msg.degraded || msg.injectionFlagged || msg.truncated || msg.persisted === false || msg.verified === false || msg.retrievalIncomplete || (msg.unreviewedUsed ?? 0) > 0 || (msg.groundingMode && msg.groundingMode !== 'standard')) && (
                <div className="mt-2 ml-11 flex flex-wrap gap-1.5">
                  {msg.fallback && (
                    <span className="inline-flex items-center gap-1 rounded bg-violet-50 px-1.5 py-0.5 text-[11px] font-medium text-violet-700 border border-violet-100">
                      Modo contingencia ({providerLabel(msg.provider)}{msg.model ? ` · ${msg.model}` : ''}) - OpenAI principal no disponible
                    </span>
                  )}
