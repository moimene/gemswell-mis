...
| **Bulk-endorse the 797** to `source_of_record` | High-severity business decision (changes what the CFO chat calls "official"); classifier-blocked under general authorization. Pipeline BUILT + verified (0→5→0) + reversible. | `node scripts/endorse-source-of-record.mjs --apply` (798 RPC calls, ~min) → source_of_record 0→797. Revert: `--revert`. **Needs explicit user "endorse all 797".** |
