-- Add provider-aware vector retrieval.
--
-- OpenAI and Gemini embeddings can share dimensionality but not vector space. The
-- vector RPC must therefore filter by the model that produced each stored vector
-- before an OpenAI query embedding can be used safely.

drop function if exists public.match_chunks(vector, integer, text, text, double precision);
  ...
  select
    ...
    coalesce(v.metadata, '{}'::jsonb)
      ...
      || jsonb_build_object(
           ...
           'embedding_model',      coalesce(v.embedding_model, v.metadata->>'embedding_model')) as metadata,
  ...
  from (
    ...
    where d.review_status <> 'rejected'
      ...
      and (
        ...
        or coalesce(c.embedding_model, c.metadata->>'embedding_model', 'gemini-embedding-001') = filter_embedding_model
      )
