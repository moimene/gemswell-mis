-- 019 — retrieval governance filter: exclude lifecycle='superseded' from chat retrieval.
--
-- WHY (audit 2026-06-07, verified live on nqxhsjkcvfxygiajdxki): both retrieval RPCs filtered only
-- `review_status <> 'rejected'` + `classification_source <> 'agent_rejected'` + `status = 'indexed'`.
-- NOTHING filtered `lifecycle`. Result: 7 documents (369 chunks) marked `lifecycle='superseded'` but
-- still `status='indexed'` (a backfill that set the lifecycle without retiring the doc) remained fully
-- retrievable — a replaced revision could be cited alongside its replacement, the exact contradiction
-- a versioned DMS exists to prevent. Confirmed: `bool_or(prosrc like '%lifecycle%superseded%') = false`.
--
-- FIX: add `and d.lifecycle is distinct from 'superseded'` to the WHERE of BOTH match_chunks (the inner
-- index-served distance scan) and keyword_search_chunks. `is distinct from` keeps NULL lifecycle rows in
-- (they are not superseded). Same signatures / return shapes / performance characteristics as sql/015
-- (HNSW iterative scan) and sql/018 (df-aware keyword) — only the governance predicate changes.
--
-- DELIBERATELY NOT CHANGED: needs_review / pending documents stay retrievable. Per the chosen policy
-- ("fallback + disclosure"), the chat keeps them as a ranked-below-approved fallback (rankBySourceTrust)
-- and the app discloses reliance (diagnostics.unreviewedUsed); hard-excluding 41% of the corpus would
-- gut recall. The real remediation for needs_review is reviewing the backlog, not a WHERE clause.
--
-- App-layer mirror: src/lib/rag/retrieve.ts isExcludedFromRetrieval() drops superseded as defense-in-depth.
-- Rollback: sql/rollback/019_rollback.sql (restores the sql/015 + sql/018 bodies without the lifecycle line).

begin;

-- ── match_chunks (vector) — sql/015 body + lifecycle filter ──────────────────
...
-- ── keyword_search_chunks (FTS) — sql/018 body + lifecycle filter ────────────
