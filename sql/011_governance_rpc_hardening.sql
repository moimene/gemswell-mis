-- 011_governance_rpc_hardening.sql — Codex adversarial pass-2 fixes (applied as `governance_rpc_hardening`).
--
-- CX-B1 (CRITICAL): apply_document_governance + knowledge_corpus_health are SECURITY DEFINER (bypass RLS).
--                   Default EXECUTE grant to PUBLIC let the anon key call them directly and forge governance.
--                   Revoke from public/anon/authenticated; grant only to service_role (the API route's client).
-- CX-B3 (HIGH): optimistic lock was inert for approve/reject/reclassify/retire/restore (current_version only
--               changed on supersede). Now EVERY governed write advances current_version (revision counter),
--               so a stale p_expected_version is detected (40001) for all actions.
-- CX-B4 (HIGH): coalesce() can't distinguish an intentional JSON null from an absent key. Nullable columns
--               (doc_type/project_id/period/supersedes_document_id) now use `p_patch ? 'key'` so a present-null
--               actually clears the column and the audit event matches reality.
-- CX-B5 (HIGH): lock both docs in a deterministic id order to avoid A<->B supersede deadlock (40P01).
-- CX-B7 (MED): health rejected/pending counts now status='indexed'-scoped like approved/needs_review.
...
declare
  ...
  v_existing uuid;
begin
  -- CX-B5: deterministic lock order (lock both rows by id) to avoid cross-supersede deadlock
  if p_related_id is not null then
    perform 1 from rag_documents where id in (p_doc_id, p_related_id) order by id for update;
  else
    perform 1 from rag_documents where id = p_doc_id for update;
  end if;

  select current_version into v_cur_version from rag_documents where id = p_doc_id;
  ...
  if p_related_id is not null then
    ...
    if not exists (select 1 from rag_documents where id = p_related_id) then
  ...
  update rag_documents set
    ...
    doc_type               = case when p_patch ? 'doc_type'   then p_patch->>'doc_type'   else doc_type   end,
    project_id             = case when p_patch ? 'project_id' then p_patch->>'project_id' else project_id end,
    period                 = case when p_patch ? 'period'     then p_patch->>'period'     else period     end,
    ...
    supersedes_document_id = case when p_patch ? 'supersedes_document_id' then (p_patch->>'supersedes_document_id')::uuid else supersedes_document_id end,
    -- CX-B3: advance revision on every governed write so the optimistic check works for all actions
    current_version        = coalesce((p_patch->>'current_version')::integer, current_version + 1)
  where id = p_doc_id;
...
-- CX-B1: lock down EXECUTE so only the service-role API client can call these RLS-bypassing functions.
revoke execute on function apply_document_governance(uuid, jsonb, integer, uuid, jsonb, jsonb) from public;
revoke execute on function knowledge_corpus_health() from public;
do $$
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    execute 'revoke execute on function apply_document_governance(uuid, jsonb, integer, uuid, jsonb, jsonb) from anon, authenticated';
    execute 'revoke execute on function knowledge_corpus_health() from anon, authenticated';
    execute 'grant execute on function apply_document_governance(uuid, jsonb, integer, uuid, jsonb, jsonb) to service_role';
