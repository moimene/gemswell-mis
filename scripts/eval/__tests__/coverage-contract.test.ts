...
describe('critical eval coverage contract', () => {
  ...
  it('keeps live-rag-e2e wired to the critical eval and browser gates', () => {
    ...
    expect(documentIngestE2e).toContain('expectedSourceIsFirst')
    expect(documentIngestE2e).toContain('allSourceLinksMatchExpectedDocument')
    expect(documentIngestE2e).toContain('answerMentionsToken')
  ...
  it('keeps prompt-behavior adversarial cases in the live checker', () => {
    ...
    expect(checker).toContain('(?:te|se)\\s+refier')
  })
})
