...
const ABSTAIN_RE = /(no\s+(?:hay|existe|se\s+(?:han?\s+)?encontr|dispongo|tengo|consta|encuentro)|sin\s+evidencia|no\s+evidence|there\s+(?:is|was)\s+no\s+(?:specific\s+)?(?:documentary\s+)?evidence|there\s+(?:is|was)\s+no\s+(?:mention|record|reference)\s+of|(?:do|did)\s+not\s+find(?:\s+\w+){0,4}\s+evidence|(?:do|did)\s+not\s+find(?:\s+\w+){0,10}\s+(?:policy|treasury|hedging)|found\s+no\s+(?:documentary\s+)?evidence|no\s+documentary\s+evidence|not\s+found|no\s+relevant|cannot\s+find|has\s+been\s+found|no\s+he\s+(?:encontrado|hallado)|no\s+.{0,90}(?:exists?|appears?|is\s+(?:found|present)|encontrad\w*)\s+in\s+the\s+.{0,25}corpus|no\s+.{0,30}(?:en\s+el\s+corpus|documental))/i
